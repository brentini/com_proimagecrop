...
$.imgAreaSelect = function (img, options) {
    ...
    function update(resetKeyPress) {
        ...
        case 8:
            jQuery($handles[4]).css({ left: w / 2 });
            jQuery($handles[5]).css({ left: w, top: h / 2 });
            jQuery($handles[6]).css({ left: w / 2, top: h });
            jQuery($handles[7]).css({ top: h / 2 });
        case 4:
        ...
        if (resetKeyPress !== false) {
            if ($.imgAreaSelect.keyPress != docKeyPress)
                jQuery(document).unbind($.imgAreaSelect.keyPress,
                    $.imgAreaSelect.onKeyPress);

            if (options.keys)
                jQuery(document)[$.imgAreaSelect.keyPress](
    ...
    function setOptions(newOptions) {
        ...
        for (i = 0; i++ < 4;)
            jQuery($border[i-1]).addClass(options.classPrefix + '-border' + i);
        ...
        if (o = options.borderColor1)
            jQuery($border[0]).css({ borderStyle: 'solid', borderColor: o });
        if (o = options.borderColor2)
            jQuery($border[1]).css({ borderStyle: 'dashed', borderColor: o });
...
$.fn.imgAreaSelect = function (options) {
    ...
    this.each(function () {
        if (jQuery(this).data('imgAreaSelect')) {
            ...
            jQuery(this).data('imgAreaSelect', new $.imgAreaSelect(this, options));
        }
    });
